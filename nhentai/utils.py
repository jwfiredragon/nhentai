    ...
    if doujinshi_list is not None:
        for doujinshi_obj in doujinshi_list:
            if os.path.exists(os.path.join(output_dir, doujinshi_obj.filename)):
                ...
                logger.warning('Gallery {doujinshi_obj.filename} does not exist, skipped')
    else:
